...
#         assert (len(self.ft_chns) == 5)
#
#         self.up1 = UpBlock_MC(self.ft_chns[4], self.ft_chns[3], self.ft_chns[3], dropout_p=0.0,
#                               mode_upsampling=self.up_type)
#         self.up2 = UpBlock_MC(self.ft_chns[3], self.ft_chns[2], self.ft_chns[2], dropout_p=0.0,
#                               mode_upsampling=self.up_type)
#         self.up3 = UpBlock_MC(self.ft_chns[2], self.ft_chns[1], self.ft_chns[1], dropout_p=0.0,
#                               mode_upsampling=self.up_type)
#         self.up4 = UpBlock_MC(self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0,
#                               mode_upsampling=self.up_type)
#
...
class Decoder_feaaddSDM(nn.Module):
    def __init__(self, params):
        super(Decoder_feaaddSDM, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, feature):
        ...
        #output = self.out_conv(x)
        out_tanh = self.tanh(output)
        return output,fea,out_tanh#, [y1, y2, y3, y4]
...
class UNet_feaaddSDM(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_feaaddSDM, self).__init__()
        ...
        self.decoder = Decoder_feaaddSDM(params)

    def forward(self, x):
        feature = self.encoder(x)
        output, fea, out_tanh = self.decoder(feature)
        return output, fea, out_tanh


class UNet_feaaddSDMCAM(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_feaaddSDMCAM, self).__init__()
        ...
        self.out_conv = nn.Conv2d(self.ft_chns[0], self.n_class,
                                  kernel_size=3, padding=1)
        self.tanh = nn.Tanh()
        self.last_conv_layer = self.out_conv
        # self.encoder = Encoder(params)
        # self.decoder = Decoder_feaaddSDM(params)
    ...
    def forward(self, x):
        ...
        x2 = self.down2(x1)
        x3 = self.down3(x2)
        self.x4 = self.down4(x3)
        y4 = self.up1(self.x4, x3)
        y3 = self.up2(y4, x2)
        y2 = self.up3(y3, x1)
        y1 = self.up4(y2, x0)
        output = self.out_conv(y1)
    ...
    def get_cam(self, class_idx):
        ...
        feature_map = self.x4
        # 获取权重（假设是1x1卷积的输出）
        weights = self.last_conv_layer.weight[class_idx].data.cpu().numpy()
        # 计算类激活图
        cam = np.zeros(feature_map.shape[2:], dtype=np.float32)
        for i in range(weights.shape[0]):
            cam += weights[i] * feature_map[0, i,...].cpu().detach().numpy() # 假设输入为batch_size=1
...
class EncoderSDMfea(nn.Module):
    def __init__(self, params):
        super(EncoderSDMfea, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, x):
        ...
        x3 = self.down3(x2)
        x4 = self.down4(x3)
        return [x0, x1, x2, x3, x4], [x0, x1, x2, x3]
...
#                   'acti_func': 'relu'}
#
#         self.encoder = EncoderSDMfea(params)
#         self.decoder = Decoder_feaaddSDM(params)
#
#     def forward(self, x):
#         feature, encoder_features = self.encoder(x)
#         output, fea, out_tanh, decoder_features = self.decoder(feature)
#         return output, fea, out_tanh, encoder_features, decoder_features
...
class UNet_fea(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_fea(params)


    def forward(self, x):
        feature = self.encoder(x)
        output,fea= self.decoder(feature)
        return output,fea
...
class UNet_CCT_CMMT(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CCT_CMMT, self).__init__()
        ...
        params = {'in_chns': in_chns,
                  ...
                  'acti_func': 'relu'}
        self.encoder = Encoder(params)
        self.main_decoder = Decoder_feaaddSDM(params)
        self.aux_decoder1 = Decoder(params)

    def forward(self, x):
        feature = self.encoder(x)
        main_seg, fea, out_tanh = self.main_decoder(feature)
        aux1_feature = [FeatureNoise()(i) for i in feature]
        aux_seg1 = self.aux_decoder1(aux1_feature)
        return main_seg, fea, aux_seg1
