...
class vnet_dv_semi(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        self.dsv4 = UnetDsv3(
            in_size=n_filters * 16, out_size=n_classes, scale_factor=8)
        self.dsv3 = UnetDsv3(
            in_size=n_filters * 8, out_size=n_classes, scale_factor=4)
        self.dsv2 = UnetDsv3(
            in_size=n_filters * 4, out_size=n_classes, scale_factor=2)
        self.dsv1 = nn.Conv3d(
            in_channels=n_filters * 2, out_channels=n_classes, kernel_size=1)
    ...
    def decoder(self, features):
        ...
        x5_up = x5_up + x4
        up4 = self.dropout1(x5_up)

        x6 = self.block_six(x5_up)
        x6_up = self.block_six_up(x6)
        x6_up = x6_up + x3
        up3 = self.dropout2(x6_up)

        x7 = self.block_seven(x6_up)
        x7_up = self.block_seven_up(x7)
        x7_up = x7_up + x2
        up2 = self.dropout3(x7_up)

        x8 = self.block_eight(x7_up)
        x8_up = self.block_eight_up(x8)
        x8_up = x8_up + x1
        up1 = self.dropout4(x8_up)
