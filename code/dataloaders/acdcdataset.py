...
from augmentations.ctaugment import blur
...
def random_rotate(image, label):
    ...
    # aug_image = ndimage.rotate(aug_image, angle, order=0, reshape=False)
    return image, label
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        aug_image = Image.fromarray((image * 255).astype(np.uint8))
        aug_image = deepcopy(aug_image)
        aug_image = transforms.ColorJitter(0.5, 0.5, 0.5, 0.25)(aug_image)
        aug_image = blur(aug_image, p=0.5)
        aug_image = torch.from_numpy(np.array(aug_image)).unsqueeze(0).float() / 255.0
        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        label = torch.from_numpy(label.astype(np.uint8))

        sample = {"image": image, "label": label, "aug_image": aug_image}
        return sample
