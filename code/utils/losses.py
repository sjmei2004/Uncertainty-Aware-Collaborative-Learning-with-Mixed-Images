...
class Attention(nn.Module):
    ...
    def forward(self, g_s, g_t):
        loss = sum([self.at_loss(f_s[12:], f_t[12:].detach()) for f_s, f_t in zip(g_s, g_t)])
        return loss
...
class BCPDiceLoss(nn.Module):
    def __init__(self, n_classes):
        super(BCPDiceLoss, self).__init__()
...
#         consis = torch.mean((input1 - input2)**2,dim=1)
#         class_losses=0
#         w=self.weight
#         for i in range(4):
#             # 索引出每个类别的数据（batch_size, 256, 256）
#             input1_class = input1[:, i, :, :]
#             input2_class = input2[:, i, :, :]
#
#             # 计算L1损失（或者L2损失：F.mse_loss）
#             loss = torch.mean((input1 - input2)**2)
#
#             # 存储损失
#             class_losses+=loss*w[i]
#         return class_losses
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, w_dc=None, w_ce=None, aggregate="sum", weight_ce=1, weight_dice=1,
        ...
        self.ce = RobustCrossEntropyLoss(**ce_kwargs)#mse_loss#(input1, input2)#
    ...
    def forward(self, net_output, target):
        ...
        ce_loss = self.ce(net_output, target[:, 0].long()) if self.weight_ce != 0 else 0#torch.mean((net_output - target) ** 2)
...
class CC_loss(nn.Module):
    def __init__(self, w_ce=None, aggregate="sum", weight_ce=1,
                 ):
        # """
        # CAREFUL. Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        # :param aggregate:
        ...
        self.weight_ce = weight_ce
        self.aggregate = aggregate
        self.ce = mse_loss(weight=w_ce)#(input1, input2)#RobustCrossEntropyLoss(**ce_kwargs)
    ...
    def forward(self, net_output, target):
        ...
        ce_loss = self.ce(net_output, target) if self.weight_ce != 0 else 0#torch.mean((net_output - target) ** 2,)
        a=ce_loss
